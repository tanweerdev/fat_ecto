defmodule FatEcto.Query.MyApp.HospitalQueryTest do
  use FatEcto.ConnCase
  import Ecto.Query
  alias FatEcto.FatHospital

  alias FatEcto.Query.MyApp.HospitalQuery

  describe "build/3" do
    test "filters by name with custom $ILIKE operator" do
      base_query = from(h in FatHospital)
      query = HospitalQuery.build(base_query, %{"name" => %{"$ILIKE" => "%General%"}})
    ...
    test "filters by name with custom $LIKE operator" do
      base_query = from(h in FatHospital)
      query = HospitalQuery.build(base_query, %{"name" => %{"$LIKE" => "%General%"}})
    ...
    test "ignores name with ignoreable value" do
      base_query = from(h in FatHospital)
      query = HospitalQuery.build(base_query, %{"name" => ""})
    ...
    test "filters by phone with custom $ILIKE operator" do
      base_query = from(h in FatHospital)
      query = HospitalQuery.build(base_query, %{"phone" => %{"$ILIKE" => "%123%"}})
    ...
    test "ignores phone with ignoreable value" do
      base_query = from(h in FatHospital)
      query = HospitalQuery.build(base_query, %{"phone" => nil})
    ...
    test "does not filter by non-overrideable fields" do
      base_query = from(h in FatHospital)
      query = HospitalQuery.build(base_query, %{"email" => "test@example.com"})
